/*
 * This code is based on https://github.com/marknutter/GCM-Cordova/blob/master/assets/www/CORDOVA_GCM_script.js
 * with slight modifications.
 *
 * It register itself with the GCM service and then handles push messages by sending them further to our
 * Angular code.
 */
gApp = new Array();
...
document.addEventListener('deviceready', function() {
  ...
  //This needs run either way if it already as been registered. -PMM
  window.plugins.GCM.register("250640310429", "GCM_Event", GCM_Success, GCM_Fail ); // the number is our Google API "project number"
 ...
 // $("#app-status-ul").append('<li>EVENT -> RECEIVED:' + e.event + '</li>');
  ...
  case 'registered':
    ...
    if ( gApp.gcmregid.length > 0 )
    {
      ...
      window.localStorage.setItem("gcmregid",e.regid); //setting GCM id to local storage, so we could get in in angular.
      //var lol = localStorage.getItem("gcmregid");
      //angular.element(document.getElementById("GCMController")).scope().$apply(function(scope){scope.onRegister(e)});
  ...
  case 'message':
    ...
    //$("#app-status-ul").append('<li>Title: ' + e.title + '</li>');
    //$("#app-status-ul").append('<li>MESSAGE: ' + e.message + '</li>');
    //angular.element(document.getElementById("allEventsApp")).scope().$apply(function(scope){scope.onGCM()});
    angular.element(document.getElementById("GCMController")).scope().$apply(function(scope){scope.onMessage(e)});
    break;
